...
public class SpectrumAnalyzer : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        {
            float level = spectrum[i]*settings.pillar.sensitivity*Time.deltaTime*500+1; //0,1 = l,r for two channels

            //Scale pillars
            Vector3 previousScale = pillars[i].transform.localScale;
            previousScale.x = Mathf.Lerp(previousScale.x, level, settings.pillar.speed*Time.deltaTime);
